...
| Données des dashboards coach / sportif / club | BLOCKED | Le parcours local est couvert en mode démonstration, mais le dashboard coach contient encore des éléments de présentation statiques (séances, profil, messages). Il faut les relier aux données persistées et les valider avec de vrais comptes avant une préproduction ouverte. |
