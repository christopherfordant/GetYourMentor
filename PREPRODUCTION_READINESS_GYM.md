...
| Parcours fonctionnels Playwright | PASS local / À revalider CI | Le 2026-09-18, 45/45 tests passent sur `desktop-chromium` et 45/45 sur `mobile-chromium` (90/90 local, mono-worker), incluant API, réservation, paiement, comptes, coach, admin, santé, limites d’entrée, bornes de créneau, déconnexion, confidentialité des réservations, minimisation bancaire, webhook invalide, catalogue vérifié, workflow club, validation MIME, limitation de fréquence et responsive. La CI distante doit être revalidée après les derniers commits. |
...
| Données des dashboards coach / sportif / club | BLOCKED | Le dashboard coach lit désormais le profil, les réservations, les paiements et les messages via les APIs ; le dashboard club contient encore des blocs de présentation statiques. Il faut terminer ce branchement et valider les espaces avec de vrais comptes avant une préproduction ouverte. |
